...
export const CityItem = ({ id, name , selectedValue , setSelectedValue }: CityItemProps) => {
    ...
    useEffect(() => {
      ...
      const initialCity = searchParams.get('location-city');
    ...
    const handleClick = (id:any) => {
      ...
      if (newValue) {
        params.set('location-city', newValue); // Set the new value if it's selected
      } else {
        params.delete('location-city'); // Remove the search param if the same button is clicked again
      }
