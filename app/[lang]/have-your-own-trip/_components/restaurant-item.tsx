...
export const RestaurantItem = ({
  ...
  return (
    <div className="flex w-full  flex-col md:flex-row p-2 md:p-6 gap-2 md:gap-4 aspect-[1/2.5] md:aspect-video bg-white">
      <div className=" flex flex-wrap md:flex-nowrap w-full md:w-[45%] min-h-[132px]">
        <div className="aspect-[1/1.2] relative w-full">
          <Link
              href={`/${lang}/have-your-own-trip/restaurant/${id}`}>
            <Image fill className=" h-full w-full  object-cover" src={image} alt="ssss" />
          </Link>
        </div>
      </div>
      <div className="flex flex-col justify-between w-full gap-2 md:w-[55%] space-y-2">
        ...
        <Link 
        href={`/${lang}/have-your-own-trip/restaurant/${id}`}
        className=" bg-inherit hover:text-[#F07148] rounded-none text-[#594636] text-xs md:text-sm leading-normal font-semibold text-center w-full capitalize opacity-80">
          see details
        </Link>
      </div>
    </div>
